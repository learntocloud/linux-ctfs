...
| Command | Description |
|---------|-------------|
| `./tests/deploy_and_test.sh aws` | Test AWS only (~15 min) |
| `./tests/deploy_and_test.sh azure` | Test Azure only (~15 min) |
| `./tests/deploy_and_test.sh gcp` | Test GCP only (~15 min) |
| `./tests/deploy_and_test.sh all` | Test all providers sequentially (~45 min) |
| `./tests/deploy_and_test.sh aws --with-reboot` | Test AWS with reboot verification (~20 min) |
| `./tests/deploy_and_test.sh all --with-reboot` | Full test suite (~60 min) |
...
After tests complete, verify all resources were destroyed to avoid unexpected charges:
...
If resources remain, manually destroy them:
```bash
